...
export class ReminderCommand extends SlashCommand {
    ...
    constructor(
        ...
        super();
        this.checkInterval
            .subscribe(_ => this.checkAndSendReminder())
    }
    ...
    public command: Omit<SlashCommandBuilder, 'addSubcommand' | 'addSubcommandGroup'> =  new SlashCommandBuilder()
        ...
        .addStringOption(option => 
            option.setName('wann')
                ...
                .addChoices([['1 Minute', '1min',], ['10 Minuten', '10min'], ['1 Stunde', '1h'], ['1 Tag', '1d']])
                .setRequired(true)
        );

    public Handle(interaction: CommandInteraction): void {
        const reminderText = interaction.options.getString('was', true);
        const reminderTime = <('1min' | '10min' | '1h' | '1d')>interaction.options.getString('wann', true);
        ...
        this.botRepository.AddReminder({message: reminderText, date: remindDate, userId})
        interaction.reply({content: `${response}\n  *${reminderText}*`});
    }

    private checkAndSendReminder(): void {
        let now = new Date();
        this.botRepository.GetReminder()
            .filter((reminder: ReminderModel) => reminder.date.valueOf() < now.valueOf())
            .forEach((reminder: ReminderModel) => {
                this.messageBroker.dispatchSendMessage({messageText: `Denk dran! \n*${reminder.message}*`, userId: reminder.userId})
