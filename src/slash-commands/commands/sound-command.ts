...
export class SoundCommand extends SlashCommand {
    ...
    public command: Omit<SlashCommandBuilder, "addSubcommand" | "addSubcommandGroup"> | SlashCommandSubcommandsOnlyBuilder = this.BuildCommand();
    ...
    private HandleAddCommand(interaction: CommandInteraction<CacheType>) {
        ...
        interaction.deferReply({ephemeral: true})
            .then(_ => {
                this.getVideoInfo(soundUrl)
                    .then(videoInfo => {
                        // const videoTitle = videoInfo.videoDetails.title
                        const videoLength = <number>videoInfo.videoDetails.lengthSeconds;
                        if (videoLength < 10) {
                            this.downloadVideo(soundUrl, filePath)
                                .then(_ => {
                                    this.logger.LogMessage(`Video heruntergeladen nach ${filePath}`, false)
                                    interaction.editReply({content: `Done`});                
                                    this.botRepository.AddSoundCommand({commandName: soundName, filePath});
                                    this.command = this.BuildCommand();
                                    this.commandRefreshed$.next();
                                })
                                .catch(error => this.logger.LogError(`Download fehlgeschlagen!`, false))
                        }
                        else {
                            interaction.editReply({content: `Done`});
                            this.botRepository.AddSoundCommand({commandName: soundName, link: soundUrl});
                            this.command = this.BuildCommand();
                            this.commandRefreshed$.next();
                        }
    ...
    private HandlePlayCommand(interaction: CommandInteraction<CacheType>) {
        const soundName = interaction.options.getString('name', true);
        let soundCommand = this.botRepository.GetSoundCommands()
            .find(soundCommand => soundCommand.commandName == soundName);
        this.messageBroker.dispatchPlaySoundToUser({userId: interaction.user.id, filePath: soundCommand.filePath, url: soundCommand.link})
        interaction.reply({content: `*${soundName}*`, ephemeral: true});
    }
    ...
    private HandleDeleteCommand(interaction: CommandInteraction<CacheType>) {
        ...
        this.commandRefreshed$.next();
        interaction.reply({content: `*${soundName}* gelöscht`, ephemeral: true});        
    }

    private BuildCommand(): Omit<SlashCommandBuilder, "addSubcommand" | "addSubcommandGroup"> | SlashCommandSubcommandsOnlyBuilder {
        let soundCommands = this.botRepository.GetSoundCommands().map(soundCommand => soundCommand.commandName);
        return new SlashCommandBuilder()
            ...
            .addSubcommand(subCommand => subCommand
                ...
                .addStringOption(option => option
                    .setName('name')
                    .setDescription('Auf welchen Command soll das Video gespielt werden?')
                    .setRequired(true)
                )
            ...
            .addSubcommand(subCommand => subCommand
                ...
                .addStringOption(option => option
                    ...
                    .addChoices(soundCommands.map(command => [command, command]))
                    .setRequired(true)
                )
            )
            .addSubcommand(subCommand => subCommand
